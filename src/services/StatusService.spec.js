...
beforeEach(() => {
  ...
  OrderRepository.getResult = vi.fn(() => [{
    message: {
      order: {
...
describe('Status Service', () => {
  it('should test getResult is called', async () => {
    await StatusService.status(req);
    expect(OrderRepository.getResult).toBeCalledWith(req.message.order.id);
  });
